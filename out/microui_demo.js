...
var ASM_CONSTS = {
  88416: () => { Module.canvas.focus(); },  
 88441: () => { Module.useWebGL = true; GLImmediate.init(); },  
 ...
 89433: () => { var canvas = document.getElementById('canvas'); canvas.addEventListener('drop', function(e) { e.preventDefault(); if (e.dataTransfer.file < 0) return; var filenamesArray = []; var count = e.dataTransfer.files.length; var drop_dir = '.rgfw_dropped_files'; Module._RGFW_mkdir(drop_dir); for (var i = 0; i < count; i++) { var file = e.dataTransfer.files[i]; var path = '/' + drop_dir + '/' + file.name.replace("//", '_'); var reader = new FileReader(); reader.onloadend = (e) => { if (reader.readyState != 2) { out('failed to read dropped file: '+file.name+': '+reader.error); } else { var data = e.target.result; Module._RGFW_writeFile(path, new Uint8Array(data), file.size); } }; reader.readAsArrayBuffer(file); var filename = stringToNewUTF8(path); filenamesArray.push(filename); Module._RGFW_makeSetValue(i, filename); } Module._Emscripten_onDrop(count); for (var i = 0; i < count; ++i) { _free(filenamesArray[i]); } }, true); canvas.addEventListener('dragover', function(e) { e.preventDefault(); return false; }, true); },  
 90459: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 90530: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 90587: () => { return window.mouseX || 0; },  
 90618: () => { return window.mouseY || 0; },  
 90649: ($0) => { var canvas = document.getElementById('canvas'); if ($0) { canvas.style.pointerEvents = 'none'; } else { canvas.style.pointerEvents = 'auto'; } },  
 90796: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 90849: () => { return window.innerWidth; },  
 90879: () => { return window.innerHeight; },  
 90910: ($0, $1) => { var ext = UTF8ToString($0, $1); var canvas = document.querySelector('canvas'); var gl = canvas.getContext('webgl') || canvas.getContext('experimental-webgl'); if (!gl) return 0; var supported = gl.getSupportedExtensions(); return supported && supported.includes(ext) ? 1 : 0; },  
 91190: () => { Module.requestFullscreen(false, true); },  
 91229: () => { Module.exitFullscreen(false, true); },  
 91265: ($0, $1) => { var element = document.getElementById("canvas"); if (element) element.style.opacity = $1; }
};
