...
var ASM_CONSTS = {
  6920: () => { Module.canvas.focus(); },  
 6945: ($0, $1, $2, $3, $4) => { var data = Module.HEAPU8.slice($0, $0 + $1 * $2 * 4); let context = document.getElementById("canvas").getContext("2d"); let image = context.getImageData(0, 0, $1, $2); image.data.set(data); context.putImageData(image, 0, $4 - $2); },  
 7180: () => { Module.useWebGL = true; GLImmediate.init(); },  
 7226: () => { window.addEventListener("keydown", (event) => { var key = stringToNewUTF8(event.key.value); var code = stringToNewUTF8(event.code); Module._RGFW_handleKeyMods(event.getModifierState("CapsLock"), event.getModifierState("NumLock"), event.getModifierState("Control"), event.getModifierState("Alt"), event.getModifierState("Shift"), event.getModifierState("Meta"), event.getModifierState("ScrollLock")); Module._RGFW_handleKeyEvent(key, code, 1); _free(key); _free(code); }, true); window.addEventListener("keyup", (event) => { var key = stringToNewUTF8(event.key.value); var code = stringToNewUTF8(event.code); Module._RGFW_handleKeyMods(event.getModifierState("CapsLock"), event.getModifierState("NumLock"), event.getModifierState("Control"), event.getModifierState("Alt"), event.getModifierState("Shift"), event.getModifierState("Meta"), event.getModifierState("ScrollLock")); Module._RGFW_handleKeyEvent(key, code, 0); _free(key); _free(code); }, true); },  
 8184: () => { var canvas = document.getElementById('canvas'); canvas.addEventListener('drop', function(e) { e.preventDefault(); if (e.dataTransfer.file < 0) return; var filenamesArray = []; var count = e.dataTransfer.files.length; var drop_dir = '.rgfw_dropped_files'; Module._RGFW_mkdir(drop_dir); for (var i = 0; i < count; i++) { var file = e.dataTransfer.files[i]; var path = '/' + drop_dir + '/' + file.name.replace("//", '_'); var reader = new FileReader(); reader.onloadend = (e) => { if (reader.readyState != 2) { out('failed to read dropped file: '+file.name+': '+reader.error); } else { var data = e.target.result; Module._RGFW_writeFile(path, new Uint8Array(data), file.size); } }; reader.readAsArrayBuffer(file); var filename = stringToNewUTF8(path); filenamesArray.push(filename); Module._RGFW_makeSetValue(i, filename); } Module._Emscripten_onDrop(count); for (var i = 0; i < count; ++i) { _free(filenamesArray[i]); } }, true); canvas.addEventListener('dragover', function(e) { e.preventDefault(); return false; }, true); },  
 9210: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 9281: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 9338: () => { return window.mouseX || 0; },  
 9369: () => { return window.mouseY || 0; },  
 9400: ($0) => { var canvas = document.getElementById('canvas'); if ($0) { canvas.style.pointerEvents = 'none'; } else { canvas.style.pointerEvents = 'auto'; } },  
 9547: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 9600: ($0, $1) => { var ext = UTF8ToString($0, $1); var canvas = document.querySelector('canvas'); var gl = canvas.getContext('webgl') || canvas.getContext('experimental-webgl'); if (!gl) return 0; var supported = gl.getSupportedExtensions(); return supported && supported.includes(ext) ? 1 : 0; },  
 9880: () => { return window.innerWidth; },  
 9910: () => { return window.innerHeight; },  
 9941: () => { Module.requestFullscreen(false, true); },  
 9980: () => { Module.exitFullscreen(false, true); },  
 10016: ($0, $1) => { var element = document.getElementById("canvas"); if (element) element.style.opacity = $1; }
};
...
var _RGFW_handleKeyEvent = Module['_RGFW_handleKeyEvent'] = (a0, a1, a2) => (_RGFW_handleKeyEvent = Module['_RGFW_handleKeyEvent'] = wasmExports['RGFW_handleKeyEvent'])(a0, a1, a2);
var _RGFW_handleKeyMods = Module['_RGFW_handleKeyMods'] = (a0, a1, a2, a3, a4, a5, a6) => (_RGFW_handleKeyMods = Module['_RGFW_handleKeyMods'] = wasmExports['RGFW_handleKeyMods'])(a0, a1, a2, a3, a4, a5, a6);
...
var dynCall_vddd = Module['dynCall_vddd'] = (a0, a1, a2, a3) => (dynCall_vddd = Module['dynCall_vddd'] = wasmExports['dynCall_vddd'])(a0, a1, a2, a3);
var dynCall_vdddd = Module['dynCall_vdddd'] = (a0, a1, a2, a3, a4) => (dynCall_vdddd = Module['dynCall_vdddd'] = wasmExports['dynCall_vdddd'])(a0, a1, a2, a3, a4);
var dynCall_vif = Module['dynCall_vif'] = (a0, a1, a2) => (dynCall_vif = Module['dynCall_vif'] = wasmExports['dynCall_vif'])(a0, a1, a2);
var dynCall_vdddddd = Module['dynCall_vdddddd'] = (a0, a1, a2, a3, a4, a5, a6) => (dynCall_vdddddd = Module['dynCall_vdddddd'] = wasmExports['dynCall_vdddddd'])(a0, a1, a2, a3, a4, a5, a6);
